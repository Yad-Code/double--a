...
import { ShoppingBag, Menu as MenuIcon, X, Instagram, Facebook } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between relative z-[130] bg-white">
        ...
        <div className="hidden lg:flex">
          <ul className="flex space-x-8 text-sm font-medium text-slate-600 uppercase tracking-widest">
            {NavLinks.map((item) => (
              <li key={item.id}>
                <a href={item.link} className="hover:text-rose-500 transition-colors relative group">
                  {item.name}
        ...
        <div className="flex-1 flex flex-col items-center justify-center space-y-10">
          ...
          <div className={`pt-12 flex flex-col items-center gap-6 transition-all duration-700 delay-500 ${isOpen ? "opacity-100" : "opacity-0"}`}>
             <div className="flex gap-8 text-slate-400">
              <Instagram size={24} className="hover:text-rose-500 transition-colors" />
              <Facebook size={24} className="hover:text-rose-500 transition-colors" />
            </div>
            <p className="text-[10px] uppercase tracking-[0.3em] text-slate-400 font-bold">Double A Artistry</p>
          </div>
        </div>
      </div>
    </nav>
